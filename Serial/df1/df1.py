...
class DF1ClientProtocol(SerialClientProtocol):
    ...
    def connectionMade(self):
        ...
        self.VARIABLES = self.config.get('SLC', 'Parameters').split(',')
        self.ALARMS = self.config.get('SLC', 'Alarms').split(',')
        self.localLogDir = self.config.get('FTP', 'localLogDir')
        self.remoteLogDir = self.config.get('FTP', 'remoteLogDir')

        self.lcOEE.start(self.config.getint('RS-232', 'OEETime'))
        self.lcAlarms.start(float(self.config.getfloat('RS-232', 'ALARMtime')))
        self.lcFTP.start(self.config.getint('FTP', 'FTPTime'))
    ...
    def reconnect(self):
        try:
            ...
            serialport.SerialPort(self, config.get('RS-232', 'host'), reactor, baudrate = config.getint('RS-232', 'baudrate'))
    ...
    def evaluateBits(self, alarmBits):
        ...
        if (bits[0]):
            ...
            # TODO: Add code to email correct people
            if (not self.notified):
                print "Emailing Engineering about Alarm"
                self.notified = True
        else:
            self.notified = False
...
def main():
    ...
    oeeLog  = LoggingLineWriter(config.get('FTP', 'localLogDir'))
