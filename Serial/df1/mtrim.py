...
class MTrimProtocol(SerialClientProtocol):
    ...
    def sendMessage(self):
        ...
        if self.value > 999:
            self.value = 1
        d1 = self.writeParameter(1, 1, self.value)
        d1.addErrback(self.errorHandler,"Error in sendMessage")
        results.append(d1)
        if self.parameter > 10:
            self.parameter = 0
        d2 = self.readParameter(1, self.parameter)
        d2.addErrback(self.errorHandler,"Error in sendMessage")
        results.append(d2)
        self.parameter += 1
        self.value += 1

        d3 = defer.gatherResults(results)
        d3.addCallback(self.printMessage)
        

    def printMessage(self, data):
        print "Wrote Parameter: ", data[0].parameter, " Returned: ", data[0].Data
        print "Read Parameter : ", data[1].parameter, " Returned: ", data[1].Data
